...
app.layout = html.Div(children=[
    ...
    dcc.Tabs([
        ...
        dcc.Tab(label='Forecast', children=[
            html.Div([
                html.H4('Central Demand Forecast '),
                dcc.Graph(
                    id='yearly-data',
                    figure=fig2,
                ),
        ...
        dcc.Tab(label='Metrics', children=[
            html.Div([
                ...
                html.H4('Central Demand Forecast Error Metrics'),
...
def update_table(selected_features, table_shape, forecast_method):
    ...
    if forecast_method != 'None':
        if predictions_df is not None:
            d = {'Date': real_data['Date'].values.ravel(),
                 'prediction': predictions_df.values.ravel(),
                 'Real_Values_19': y2.values.ravel()}

            fig = px.line(d, x='Date', y=['prediction', 'Real_Values_19'])
            fig.update_layout(yaxis_title='')

            diff = y2.values.ravel() - predictions_df.values.ravel()  # MBE
    ...
    default_fig.update_layout(yaxis_title='')
...
#Tentei fazer o ultimo ponto da metrica do segundo projeto no entanto tentei de várias formas de o fazer e mesmo assim não deu,
#no Dashboard.Render dizia que o unico erro que poderia ter era que o gunicorn não estava no requiremnet.txt mas eu fui ver e tenho
#prova no requirement.txt que está neste ficheiro mas mesmo assim não deixou criar o server.
#Envio aqui também o link para a minha conta Github onde tenho o projeto 2 com tudo pronto para ser usado no server que
#pretendia,
#Github: https://github.com/Almnf1/2nd-Project